<?php
    ...
    require 'config.php';
        ...
        $gaSql['link'] =  mysql_pconnect( $gaSql['server'], $gaSql['user'], $gaSql['password']  ) or
                die( $lang['server_processing']['unable_to_connect'] );

        mysql_select_db( $gaSql['db'], $gaSql['link'] ) or
                die( $lang['server_processing']['unable_to_select']. $gaSql['db'] );
        ...
        while ( $aRow = mysql_fetch_array( $rResult ) )
        {
                ...
                if((time() < $timeCreated + $auctionDurationSec)|| ($quantity == 0)){
                        ...
                        if ($isAdmin == true){
                                $row[] = "<td><a class='button' href='scripts/cancelAuctionAdmin.php?id=".$aRow[ $aColumns[5] ]."'>".$lang['server_processing']['form_cancel']."</a></td>";
                        }
                        $output['aaData'][] = $row;
                }else
                {
